...
flatten-tool unflatten --input-format csv --root-list-path "grants" "$la_fundglos_flattened_output_dir" | jq -c '.grants[]' > "$mapped_data_dir/fundglos-la-grants.jsonl"
...
flatten-tool unflatten --input-format csv --root-list-path "grants" "$fundglos_gcf_summerfield_grants_dir_for_flattening" | jq -c '.grants[]' > "$mapped_data_dir/fundglos-gcf-summerfield-grants.jsonl"
