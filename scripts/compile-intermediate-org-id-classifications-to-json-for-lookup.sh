...
# It will first merge all the CSV files representing the classification mappings into a single giant csv file, then feed this to jq, jq will then group everything by org_id so we end up with an array of lookup objects of org_id -> an array of classifications from all the schemes. This will make it relatively rapid to add classifications to organisations in the final dataset
...
csvstack $source_data_dir/*.csv | csvjson | jq --compact-output --from-file "$filter_file" > "$source_data_dir/classifications-lookup.json"
